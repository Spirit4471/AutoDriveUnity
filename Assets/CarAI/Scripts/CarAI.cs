...
public class CarAI : MonoBehaviour
{
    ...
    public GameObject steeringWheel; // Assign the steering wheel through the inspector
    public float KPH;
    ...
    private int Fails;
    private float MovementTorque = 2;
...
// define the direction of the raycast
    ...
    // define the maximum distance of the raycast
    float maxDistance = 100f;

    // define a RaycastHit variable to store information about what the raycast hits
    RaycastHit hit;

    // perform the raycast
    if (Physics.Raycast(transform.position, direction, out hit, maxDistance))
    {
        // if the raycast hits an AI_Vehicle
        if (hit.collider.tag == "AI_Vehicle")
        {
            Debug.Log("Car (" + this.name + "): Meet another vehicle");
            MaxRPM = 300; // slow down

            // code with change direction
    ...
    void ApplySteering() // Applies steering to the Current waypoint
    {
        ...
        float steeringAngle = frontLeft.steerAngle;
        steeringWheel.transform.localRotation = Quaternion.Euler(0, 0, -steeringAngle);
    }
...
void OnTriggerEnter(Collider other)
{
    ...
    {
        Debug.Log("Car (" + this.name + "): Traffic light detected");
        MaxRPM = 300; // slow down
    }
    else if (other.tag == "AI_Vehicle")
    {
        Debug.Log("Car (" + this.name + "): Meet another vehicle");
        MaxRPM = 300; // slow down
...
{
    if (other.tag == "TrafficLight" || other.tag == "Vehicle")
    {
        MaxRPM = 600; // return to normal speed
    }
}
